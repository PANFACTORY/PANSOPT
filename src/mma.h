 ...
 * @brief Method of Moving Asymptotes
 * @version 0.1
 * @date 2022-10-28
 *
...
class MMA {
   public:
    ...
    /**
     * @brief Construct a new MMA object
     *
     ...
     */
    MMA(int _n, int _m, T _a0, std::vector<T> _a, std::vector<T> _c,
        std::vector<T> _d, const std::vector<T>& _xmin,
        const std::vector<T>& _xmax)
        : n(_n), m(_m) {
    ...
    bool IsConvergence(T _currentf0) {
        return fabs((_currentf0 - this->previousvalue) /
                    (_currentf0 + this->previousvalue)) < this->epsvalue;
    }
    ...
    void ImportSetting(std::string _fname) {
        ...
        for (int j = 0; j < this->n; ++j) {
            fin >> this->xmin[j] >> this->xmax[j] >> this->xkm2[j] >>
                this->xkm1[j] >> this->L[j] >> this->U[j];
        }
    }
    ...
    void ExportSetting(std::string _fname) {
        ...
        for (int i = 0; i < this->m; ++i) {
            fout << this->a[i] << "\t" << this->c[i] << "\t" << this->d[i]
                 << std::endl;
        }

        for (int j = 0; j < this->n; ++j) {
            fout << this->xmin[j] << "\t" << this->xmax[j] << "\t"
                 << this->xkm2[j] << "\t" << this->xkm1[j] << "\t" << this->L[j]
                 << "\t" << this->U[j] << std::endl;
        }
    }
    ...
    void UpdateVariables(std::vector<T>& _xk, T _f, const std::vector<T>& _dfdx,
        ...
        if (this->k < 2) {
            for (int j = 0; j < this->n; j++) {
                this->L[j] =
                    _xk[j] - this->asyinit * (this->xmax[j] - this->xmin[j]);
                this->U[j] =
                    _xk[j] + this->asyinit * (this->xmax[j] - this->xmin[j]);
                this->L[j] = std::min(
                    std::max(_xk[j] - 10.0 * (this->xmax[j] - this->xmin[j]),
                             this->L[j]),
                    _xk[j] - 0.01 * (this->xmax[j] - this->xmin[j]));
                this->U[j] = std::min(
                    std::max(_xk[j] + 0.01 * (this->xmax[j] - this->xmin[j]),
                             this->U[j]),
                    _xk[j] + 10.0 * (this->xmax[j] - this->xmin[j]));
            }
        } else {
            for (int j = 0; j < this->n; j++) {
                T tmp =
                    (_xk[j] - this->xkm1[j]) * (this->xkm1[j] - this->xkm2[j]);
                if (tmp < T()) {
                    this->L[j] =
                        _xk[j] - this->asydecr * (this->xkm1[j] - this->L[j]);
                    this->U[j] =
                        _xk[j] + this->asydecr * (this->U[j] - this->xkm1[j]);
                } else if (tmp > T()) {
                    this->L[j] =
                        _xk[j] - this->asyincr * (this->xkm1[j] - this->L[j]);
                    this->U[j] =
                        _xk[j] + this->asyincr * (this->U[j] - this->xkm1[j]);
                } else {
        ...
        for (int j = 0; j < this->n; j++) {
            alpha[j] = std::max(
                {this->xmin[j],
                 this->L[j] + this->albefa * (_xk[j] - this->L[j]),
                 _xk[j] - this->move * (this->xmax[j] - this->xmin[j])});
            beta[j] = std::min(
                {this->xmax[j],
                 this->U[j] - this->albefa * (this->U[j] - _xk[j]),
                 _xk[j] + this->move * (this->xmax[j] - this->xmin[j])});
        }
        ...
        for (int j = 0; j < this->n; j++) {
            ...
            p0[j] = pow(this->U[j] - _xk[j], 2.0) *
                    (1.001 * dfdxp + 0.001 * dfdxm +
                     this->raa0 / (this->xmax[j] - this->xmin[j]));
            q0[j] = pow(_xk[j] - this->L[j], 2.0) *
                    (0.001 * dfdxp + 1.001 * dfdxm +
                     this->raa0 / (this->xmax[j] - this->xmin[j]));
        }
        ...
        for (int i = 0; i < this->m; i++) {
            for (int j = 0; j < this->n; j++) {
                ...
                p[this->IdxG(i, j)] =
                    pow(this->U[j] - _xk[j], 2.0) *
                    (1.001 * dfdxp + 0.001 * dfdxm +
                     this->raa0 / (this->xmax[j] - this->xmin[j]));
                q[this->IdxG(i, j)] =
                    pow(_xk[j] - this->L[j], 2.0) *
                    (0.001 * dfdxp + 1.001 * dfdxm +
                     this->raa0 / (this->xmax[j] - this->xmin[j]));
                b_buffer[i] += p[this->IdxG(i, j)] / (this->U[j] - _xk[j]) +
                               q[this->IdxG(i, j)] / (_xk[j] - this->L[j]);
            }
        }
#ifdef _USE_MPI_DEFINES
        MPI_Allreduce(b_buffer.data(), b.data(), this->m, MPI_DOUBLE, MPI_SUM,
                      MPI_COMM_WORLD);
#endif
        for (int i = 0; i < this->m; i++) {
#ifdef _USE_MPI_DEFINES
        ...
        for (int l = 0; eps > 1.0e-7; l++) {
            ...
            for (int i = 0; i < this->m; i++) {
                for (int j = 0; j < this->n; j++) {
                    G[this->IdxG(i, j)] =
                        p[this->IdxG(i, j)] / pow(this->U[j] - x[j], 2.0) -
                        q[this->IdxG(i, j)] / pow(x[j] - this->L[j], 2.0);
                }
            }

            for (int j = 0; j < this->n; j++) {
                Dx[j] = 2.0 * plambda[j] / pow(this->U[j] - x[j], 3.0) +
                        2.0 * qlambda[j] / pow(x[j] - this->L[j], 3.0) +
                        gsi[j] / (x[j] - alpha[j]) + ita[j] / (beta[j] - x[j]);
                deltilx[j] = plambda[j] / pow(this->U[j] - x[j], 2.0) -
                             qlambda[j] / pow(x[j] - this->L[j], 2.0) -
                             eps / (x[j] - alpha[j]) + eps / (beta[j] - x[j]);
            }
            ...
            for (int i = 0; i < this->m; i++) {
                ...
                for (int j = 0; j < this->n; j++) {
                    deltillambda_buffer[i] +=
                        p[this->IdxG(i, j)] / (this->U[j] - x[j]) +
                        q[this->IdxG(i, j)] / (x[j] - this->L[j]);
                }
            }
#ifdef _USE_MPI_DEFINES
            MPI_Allreduce(deltillambda_buffer.data(), deltillambda.data(),
                          this->m, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
#endif
            for (int i = 0; i < this->m; i++) {
#ifdef _USE_MPI_DEFINES
                deltillambda[i] +=
                    -this->a[i] * z - y[i] - b[i] + eps / lambda[i];
#else
                deltillambda[i] = deltillambda_buffer[i] - this->a[i] * z -
                                  y[i] - b[i] + eps / lambda[i];
#endif
            ...
            if (this->n > this->m) {
                std::vector<T> A_buffer((this->m + 1) * (this->m + 1), T()),
                    A((this->m + 1) * (this->m + 1), T());
                for (int ii = 0; ii < this->m; ii++) {
                    for (int jj = 0; jj < this->m; jj++) {
                        for (int kk = 0; kk < this->n; kk++) {
                            A_buffer[this->IdxAm(ii, jj)] +=
                                G[this->IdxG(ii, kk)] * G[this->IdxG(jj, kk)] /
                                Dx[kk];
                        }
                    }
                }
#ifdef _USE_MPI_DEFINES
                MPI_Allreduce(A_buffer.data(), A.data(),
                              (this->m + 1) * (this->m + 1), MPI_DOUBLE,
                ...
                for (int ii = 0; ii < this->m; ii++) {
                    for (int jj = 0; jj < this->n; jj++) {
                        B_buffer[ii] -=
                            G[this->IdxG(ii, jj)] * deltilx[jj] / Dx[jj];
                    }
                }
#ifdef _USE_MPI_DEFINES
                MPI_Allreduce(B_buffer.data(), B.data(), this->m, MPI_DOUBLE,
                              MPI_SUM, MPI_COMM_WORLD);
#endif
                for (int ii = 0; ii < this->m; ii++) {
#ifdef _USE_MPI_DEFINES
                ...
                B[this->m] = deltilz;
                std::vector<T> dlambdaz = MMA<T>::gauss(
                    A, B, [&](int _i, int _j) { return this->IdxAm(_i, _j); });
                for (int i = 0; i < this->m; i++) {
                ...
                for (int ii = 0; ii < this->n; ii++) {
                    for (int jj = 0; jj < this->n; jj++) {
                        for (int kk = 0; kk < this->m; kk++) {
                            A[this->IdxAn(ii, jj)] += G[this->IdxG(kk, ii)] *
                                                      G[this->IdxG(kk, jj)] /
                                                      Dlambday[kk];
                        }
                    }
                    ...
                    for (int jj = 0; jj < this->m; jj++) {
                        A[this->IdxAn(ii, this->n)] -=
                            G[this->IdxG(jj, ii)] * this->a[jj] / Dlambday[jj];
                        A[this->IdxAn(this->n, ii)] -=
                            G[this->IdxG(jj, ii)] * this->a[jj] / Dlambday[jj];
                        A[this->IdxAn(this->n, this->n)] +=
                            this->a[jj] * this->a[jj] / Dlambday[jj];
                    }
                }
                ...
                for (int ii = 0; ii < this->n; ii++) {
                    B[ii] = -deltilx[ii];
                    for (int jj = 0; jj < this->m; jj++) {
                        B[ii] -= G[this->IdxG(jj, ii)] * deltillambday[jj] /
                                 Dlambday[jj];
                    }
                }
                B[this->n] = -deltilz;
                for (int jj = 0; jj < this->m; jj++) {
                    B[this->n] +=
                        this->a[jj] * deltillambday[jj] / Dlambday[jj];
                }
                std::vector<T> dxz = MMA<T>::gauss(
                    A, B, [&](int _i, int _j) { return this->IdxAn(_i, _j); });
                for (int j = 0; j < this->n; j++) {
                    dx[j] = dxz[j];
                }
                dz = dxz[this->n];
                for (int i = 0; i < this->m; i++) {
                    ...
                    for (int j = 0; j < this->n; j++) {
                        dlambda_buffer[i] +=
                            G[this->IdxG(i, j)] * dx[j] / Dlambday[i];
                    }
                }
#ifdef _USE_MPI_DEFINES
                MPI_Allreduce(dlambda_buffer.data(), dlambda.data(), this->m,
                              MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
#endif
                for (int i = 0; i < this->m; i++) {
#ifdef _USE_MPI_DEFINES
                    dlambda[i] += -this->a[i] * dz / Dlambday[i] +
                                  deltillambday[i] / Dlambday[i];
#else
                    dlambda[i] = dlambda_buffer[i] -
                                 this->a[i] * dz / Dlambday[i] +
                                 deltillambday[i] / Dlambday[i];
#endif
                }
            }
            ...
            for (int j = 0; j < this->n; j++) {
                dgsi[j] = -gsi[j] * dx[j] / (x[j] - alpha[j]) - gsi[j] +
                          eps / (x[j] - alpha[j]);
                dita[j] = ita[j] * dx[j] / (beta[j] - x[j]) - ita[j] +
                          eps / (beta[j] - x[j]);
            }
            ...
            for (int j = 0; j < this->n; j++) {
                T txmaxj = std::max({-1.01 * dx[j] / (x[j] - alpha[j]),
                                     1.01 * dx[j] / (beta[j] - x[j]),
                                     -1.01 * dgsi[j] / gsi[j],
                                     -1.01 * dita[j] / ita[j]});
...
#ifdef _USE_MPI_DEFINES
            MPI_Allreduce(&txmax_buffer, &txmax, 1, MPI_DOUBLE, MPI_MAX,
                          MPI_COMM_WORLD);
#else
            txmax = txmax_buffer;
#endif
            ...
            for (int i = 0; i < this->m; i++) {
                T tymaxi = std::max(
                    {-1.01 * dy[i] / y[i], -1.01 * dlambda[i] / lambda[i],
                     -1.01 * dmu[i] / mu[i], -1.01 * ds[i] / s[i]});
                if (tymax < tymaxi) {
                    tymax = tymaxi;
                }
            }
            T tau = 1.0 / std::max({1.0, txmax, tymax, -1.01 * dz / z,
                                    -1.01 * dzeta / zeta});
            T deltawl = this->KKTNorm(x, y, z, lambda, gsi, ita, mu, zeta, s,
                                      eps, p, q, p0, q0, alpha, beta, b);
            for (int ll = 0; ll < 50; ll++) {
                ...
                T deltawlp1 =
                    this->KKTNorm(xpdx, ypdy, zpdz, lambdapdlambda, gsipdgsi,
                                  itapdita, mupdmu, zetapdzeta, spds, eps, p, q,
                                  p0, q0, alpha, beta, b);
            ...
            //.....Update epsl.....
            T deltawlp1 = this->KKTNorm(x, y, z, lambda, gsi, ita, mu, zeta, s,
                                        eps, p, q, p0, q0, alpha, beta, b);
    ...
    T KKTNorm(const std::vector<T>& _x, const std::vector<T>& _y, T _z,
              ...
              const std::vector<T>& _s, T _eps, const std::vector<T>& _p,
              const std::vector<T>& _q, const std::vector<T>& _p0,
              const std::vector<T>& _q0, const std::vector<T>& _alpha,
              const std::vector<T>& _beta, const std::vector<T>& _b) {
...
#ifdef _USE_MPI_DEFINES
                g_buffer[i] += _p[this->IdxG(i, j)] / (this->U[j] - _x[j]) +
                               _q[this->IdxG(i, j)] / (_x[j] - this->L[j]);
#else
                g[i] += _p[this->IdxG(i, j)] / (this->U[j] - _x[j]) +
                        _q[this->IdxG(i, j)] / (_x[j] - this->L[j]);
#endif
            }
        }
#ifdef _USE_MPI_DEFINES
        MPI_Allreduce(g_buffer.data(), g.data(), this->m, MPI_DOUBLE, MPI_SUM,
                      MPI_COMM_WORLD);
#endif

        //----------Equation(5.9a)(5.9e)(5.9f)----------
        for (int j = 0; j < this->n; j++) {
            norm_buffer += pow(plambda[j] / pow(this->U[j] - _x[j], 2.0) -
                                   qlambda[j] / pow(_x[j] - this->L[j], 2.0) -
                                   _gsi[j] + _ita[j],
                               2.0);  //  Equation(5.9a)
            norm_buffer += pow(_gsi[j] * (_x[j] - _alpha[j]) - _eps,
                               2.0);  //  Equation(5.9e)
            norm_buffer += pow(_ita[j] * (_beta[j] - _x[j]) - _eps,
                               2.0);  //  Equation(5.9f)
        }
#ifdef _USE_MPI_DEFINES
        MPI_Allreduce(&norm_buffer, &norm, 1, MPI_DOUBLE, MPI_SUM,
                      MPI_COMM_WORLD);
#else
        norm = norm_buffer;
#endif
        ...
        for (int i = 0; i < this->m; i++) {
            norm += pow(this->c[i] + this->d[i] * _y[i] - _lambda[i] - _mu[i],
                        2.0);  //  Equation(5.9b)
            norm += pow(g[i] - this->a[i] * _z - _y[i] + _s[i] - _b[i],
                        2.0);                             //  Equation(5.9d)
        ...
        //----------Equation(5.9c)(5.9h)----------
        norm += pow(this->a0 - _zeta -
                        std::inner_product(_lambda.begin(), _lambda.end(),
                                           this->a.begin(), T()),
                    2.0);                     //  Equation(5.9c)
        ...
        for (int i = 0; i < int(_b.size()) - 1; i++) {
            ...
            for (int j = i + 1; j < _b.size(); j++) {
                for (int k = i + 1; k < _b.size(); k++) {
                    _A[_idx(j, k)] -=
                        _A[_idx(i, k)] * _A[_idx(j, i)] / _A[_idx(i, i)];
                }
